patches-own [
  contribution ;; 1 for cooperation, 0 for defection
...
to setup
  clear-all
  ask patches [
    ifelse random 100 < init-defectors-percent [
      set pcolor red
      set contribution 0
      set wealth 10
    ][
...
to play-pgg
  set current-contribution  ( (random-float 0.002 - 0.001) + reward-factor) * (count neighbors with [ pcolor = white ] )
