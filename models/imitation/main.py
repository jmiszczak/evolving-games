...
if __name__ == '__main__':
    ...
    if len(argv) < 2:
        print(
            "[ERROR] Please select 'run', 'prun' or 'plot' as the action.")
        exit(-1)
    else:
        # chose the action
        cmd = argv[1]  # can be 'run', 'prun', or 'plot'
    ...
    if len(argv) == 2:
        if cmd == 'plot':
            print("[INFO]", cmd_info, len(listdir(ex_name)), "experiments.",
                  file=stderr)
        else:
        ...
        print("[INFO]", cmd_info,
              "experiments in range {:05d}-{:05d}".format(rs, re), file=stderr)
        ex_range = range(rs, re)
        ex_range_name = "{:05d}-{:05d}".format(rs, re)
    else:
        print("[INFO]", cmd_info, "with fallback to the default range.",
              file=stderr)
        ex_range = range(ex_count)
    ...
    if cmd == 'plot':
        ...
        # read data from the file using the same formatting
